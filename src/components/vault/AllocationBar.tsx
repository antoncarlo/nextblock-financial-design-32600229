...
export function AllocationBar({ policies }: AllocationBarProps) {
  ...
  const totalWeight = policies.reduce(
    (sum, p) => sum + Number(p.vault.allocationWeight), 0,
  );

  return (
    <div>
      <h3 className="mb-2 text-sm font-medium text-gray-700">Policy Allocation</h3>
      <div className="flex h-3 w-full overflow-hidden rounded-full">
        {policies.map((p) => {
          const widthPct = totalWeight > 0 ? (Number(p.vault.allocationWeight) / totalWeight) * 100 : 0;
          const color = SEGMENT_COLORS[p.global.verificationType] ?? 'bg-gray-300';
          return (
            <div
          ...
          return (
            <div key={Number(p.policyId)} className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-gray-600">
                ...
                <span className="font-mono-num font-medium">{formatAllocationWeight(p.vault.allocationWeight)}</span>
              </span>
            </div>
          );
        })}
